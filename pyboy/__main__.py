...
parser.add_argument(
    "--profiling", action="store_true", help="Enable opcode profiling (internal use)"
)
parser.add_argument(
    "--randomize-ram", action="store_true", help="Randomize Game Boy RAM on startup"
)
parser.add_argument(
    "--log-level",
    default="INFO",
...
"""
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect(("127.0.0.1", 55722))

        # Temp - limit listening range to just relevant ranges for testing.
        to_listen = (
            list(range(0xD2F7, 0xD31D))
            + list(range(0xDBE4, 0xDC24))
            + list(range(0xDE99, 0xDED9))
        )
